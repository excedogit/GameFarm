...
class AccountAgedTrialBalanceWebkit(PartnersOpenInvoicesWebkit):
    ...
    def __init__(self, cursor, uid, name, context=None):
        ...
        cursor.execute("select value_reference,res_id from ir_property where name='property_payment_term'")
        PAYMENT_IDS.append(cursor.fetchall())

        PAYMENT_NOTES=[]
        cursor.execute("select id,name from account_payment_term")
        PAYMENT_NAMES.append(cursor.fetchall())

        cursor.execute('select id,payment_note,payment_next_Action_date from res_partner')
        PAYMENT_NOTES.append(cursor.fetchall())

        #print "Testing the Variable Name Here :::::", name
        #print "Calling the Class for Payment Notes Update Every Time:::"
        for PAYMENT_NOTE in PAYMENT_NOTES[0]:
            PAYMENT_VAR[PAYMENT_NOTE[0]]={'id':PAYMENT_NOTE[0],'payment_note':PAYMENT_NOTE[1],'payment_next_Action_date':PAYMENT_NOTE[2]}
    ...
    def set_context(self, objects, data, ids, report_type=None):
        ...
        for acc in self.objects:
            ...
            for part_id, partner_lines in acc.ledger_lines.items():
                ...
                if PAYMENT_IDS:
                    pay_val={}
                    for values in PAYMENT_IDS:
                        for value in values:
                            pay_val[int(value[1].split(',')[-1])] = value[0].split(',')[-1]
                        for values in PAYMENT_NAMES[0]:
                            if part_id in pay_val.keys():
                                if int(pay_val[part_id])==int(values[0]):
                                    payment_ids[part_id] = values[1]
                            else:
                                payment_ids[part_id] = None
                    payment_info[part_id]={'payment_term':payment_ids[part_id],'payment_note':PAYMENT_VAR[part_id]['payment_note'],'payment_next_Action_date':PAYMENT_VAR[part_id]['payment_next_Action_date']}

                data['form']['payment_info'] = payment_info
    ...
    def compute_aged_lines(self, partner_id, ledger_lines, data):
       ...
       # print "Aged Lines Before Here :::::", aged_lines
        ...
        #    print "Testing the Line Here ::::", line
        ...
        #    print "Classification Variable Here :::",classification
            aged_lines[classification] += line['debit'] - line['credit']
            #aged_lines['payment_term']=payment_ids[partner_id]

           # print "Aged Lines Here in compute aged Balance :::", aged_lines
