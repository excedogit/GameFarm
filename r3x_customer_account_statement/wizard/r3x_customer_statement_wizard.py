...
class r3x_customer_statement_stat_by_partner(osv.osv):
    _name = "r3x_customer_statement.stat.by.partner"
    _description = "Csustomer Statement by Partner"
    ...
    def init(self, cr):
        tools.drop_view_if_exists(cr, 'r3x_customer_statement_stat_by_partner')
        ...
        cr.execute("""
            create or replace view r3x_customer_statement_stat_by_partner as (
                SELECT
...
r3x_customer_statement_stat_by_partner()


class r3x_customer_statement_sending_results(osv.osv_memory):

    def do_report(self, cr, uid, ids, context=None):
        if context is None:
            context = {}
        print "Context In the Report ::::",context
        print "Omkar want to Modify the Report Part Here............................!!!"
        datas = {
             'ids': [],
             'model': 'r3x_customer_statement',
             'form': {'partner_ids':context.get('partner_ids','')},
        }
        print "Testing The Datas Once Here::",datas
        return {
        ...
        #return context.get('r3x_customer_statement_print')
    ...
    _name = 'r3x_customer_statement.sending.results'
...
r3x_customer_statement_sending_results()


class r3x_customer_statement_print(osv.osv_memory):
    _name = 'r3x_customer_statement.print'
    ...
    _columns = {
        ...
        'partner_ids': fields.many2many('r3x_customer_statement.stat.by.partner', 'partner_stat_rel',
    ...
    def process_partners(self, cr, uid, partner_ids, data, context=None):
        ...
        nbprints = 0
        resulttext = " "
        for partner in self.pool.get('r3x_customer_statement.stat.by.partner').browse(cr, uid, partner_ids, context=context):
            ...
            if partner.max_followup_id.send_letter:
                ...
                nbprints += 1
                message = _("Customer Statement of ") + "<I> " + partner.partner_id.latest_followup_level_id_without_lit.name + "</I>" + _(" will be sent")
    ...
    def clear_manual_actions(self, cr, uid, partner_list, context=None):
        ...
        partner_list_ids = [partner.partner_id.id for partner in self.pool.get('r3x_customer_statement.stat.by.partner').browse(cr, uid, partner_list, context=context)]
    ...
    def do_process(self, cr, uid, ids, context=None):
        ...
        model_data_ids = mod_obj.search(cr, uid, [('model','=','ir.ui.view'),('name','=','view_r3x_customer_statement_sending_results')], context=context)
        ...
        return {
            ...
            'context': context,
            'view_mode': 'tree,form',
            'res_model': 'r3x_customer_statement.sending.results',
    ...
    def _get_partners_followp(self, cr, uid, ids, context=None):
        ...
        cr.execute(
            "SELECT * "\
            "FROM r3x_customer_statement_followup_line "\
            "WHERE followup_id=%s "\
